...
int audio_input;
long RGB_MusicaA;
byte FrequenzaAggiornamento = 2;
byte decay_check = 0;

const int numCampionamenti = 2;
int media;
int MaxValueAudio = 1023;
...
void RGB_Musica() {
  ...
  for (int i = 0; i < 9; i++) {
    int audioReact = map(audio_input, media, MaxValueAudio, 0, NUM_LEDS_OUT[i + 1]);
   // Audio_pre_react[i] = map(audio_input, media, MaxValueAudio, 0, NUM_LEDS_OUT[i + 1]);
   // Audio_react[i] = max(Audio_react[i], Audio_pre_react[i]);
    
    if (++decay_check > FrequenzaAggiornamento) {
      decay_check = 0;
      if (audioReact > 0) audioReact--;
    }
